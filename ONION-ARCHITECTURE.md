# Onion Architecture
Привет.  
Я написал статью после того, как в [Бэкенд академи](https://gpb.fut.ru/itfactory/backend) показал студентам пример проектирования простого приложения.  

Я обратил внимание, что не просто донести суть разделения на слои приложения новичкам.   
Вопросы, которые мне казались простыми, совсем не очевидны джунам, что логично, но не всегда очевидны старшему разработчику.  

В статье я собрал описание Луковой архитектуры,  
акцентировал внимание на важной идее про отделение бизнес-логики приложения от базы данных.  
Ответил на вопросы:  
...
Описанный подход в статье, работает в реальных проектах.  
...
Код упрощен для понимания подхода, далее он будет эволюционировать в сторону декларативного описания бизнес-логики.  
...
Впервые о Луковой Архитектуре написал [в статье Jeffrey Palermo в 2008-о](https://jeffreypalermo.com/2008/07/the-onion-architecture-part-1/)
...
В статье мы углубимся в ключевые концепции луковой архитектуры.   
...
Самое важное для нас - **Модель предметной области**, которая расположена в самом центре.
...
Это так называемый уровень Шлюзов и Адаптеров. 
**Важно**, обратите внимание: поведение сохранения объектов (ClientGateway) не является ядром приложения, 
...
Мы можем легко доработать приложение, добавив интерфейс взаимодействия с приложением по REST API.  
...
Этот класс привязан к определенному методу доступа к данным и находится вне ядра приложения. 

## Правило зависимости:

Зависимости текут внутрь, при этом внутренние уровни не имеют информации о внешних слоях. Это гарантирует, что модули в центре, которые реализуют бизнес-логику, не зависят напрямую от модулей уровня инфраструктуры. Это обеспечивает взаимозаменяемость внешних реализаций и уменьшает связность бизнес-логики и инфры.
...
Это очень важный сдвиг парадигмы восприятия проектирования приложения. 
Много новичков и старичков привыкли воспринимать приложения, как «приложения баз данных».  
Разработчики часто упускают, что самое важное для нас - моделирование бизнес-процесса и тестирование бизнес-процесса.  
...
Отделение приложения от базы данных, **позволяет сконцентрироваться на моделировании бизнес-процесса** и описать бизнес-логику (use-case), бизнес-правила (business-rules) объектами.   
...
100% покрытие бизнес-логики юнит-тестами - это недооцененный подарок данного подхода к проектированию. Многие разработчики не видят это преимущество.  
Подход ведет нас прямиком к сбалансированной Пирамиде Тестирования, надежному софту и качественному ИТ производству.  

Качество достигается за счет минимизации времени на сборку и тестирование, что, в свою очередь, приводит к сокращению ТТМ (Time to Market) и сокращению количества инцидентов, которые связаны с ошибками проектирования и тестирования.  
...
Гексагональная архитектура и луковая архитектура имеют общую предпосылку:  
вынести инфраструктуру наружу и написать код адаптера, чтобы инфраструктура не становилась тесно связанной.  

В Луковой Архитектуре впервые акцент был направлен на проектирование бизнес-логики, а уровень хранения объектов - это просто внешний слой.
...
Такое разделение повышает удобство сопровождения кода и облегчает тестирование самого важного - кода, который таит в себе бизнес-логику.  
...
- Уровень представления (Presentation Layer): Управляет пользовательскими интерфейсами. Точка входа нашего приложения для запуска бизнес-процесса.

Например, REST API, терминал.
...
Реализует взаимодействие с внешними зависимости.  
...
делая его более восприимчивым к меняющимся требованиям.

### Тестируемость:
Каждый уровень может быть протестирован независимо модульными тестами (unit tests).
Этот факт гарантирует, что бизнес-логика остается изолированной от внешних зависимостей и **покрыта юнит-тестами на 100%**.

### Гибкость
Модульная конструкция облегчает внедрение новых фреймворков и инструментов (переход с IBM MQ, Kafka), 
...
1. Бизнес логика описана в неизменяемых классах и тестируется юнит-тестами.
...
## 2.2 Шлюзы: 
Уровень шлюзов, адаптеров обеспечивает взаимодействие с внешними системами (веб сервисы, очереди, и т.п.) 
...
Я хочу остановиться на отличиях Шлюза от Адаптера.   
...
Есть графики у Владимира Хорикова и [Мартина Фоллера](https://martinfowler.com/articles/is-quality-worth-cost.html), которые наглядно показывают, что на начальном этапе накидать говна и палок - быстро и дешево, но в долгосрочной перспективе это не выгодно и стоимость поддержки возрастает. 
Это связано с тем, что сложность плохого кода растет со временем.  

![control-flow.png](visual/code-quality.png)

Проверено мной на практике.  
Именно так это работает. Делайте чаще релизы - улучшайте решение со временем.

Качественное решение уверенно можно развивать и поддерживать не теряя времени на доставку, не плодя ошибок проектирования и тестирования.
Я понимаю, что сразу делать правильно не просто. 
В любом мастерстве есть базовые кирпичики, без которых не будет развития.  
...
Есть одна вещь, которую мы уяснили о связывании за последние десятилетия, связывание - это лучшее, что может тебя замедлить в разработке.  

Связанные релизы - это то, что тебя замедляет.  
Знай это.  
