...
мы можем легко доработать приложение и добавить интерфейс взаимодействия по REST API, а для консоли напишем адаптер работы с REST-ом.
...
2. Сервисный класс не содержит бизнес-правила, он описывает поток исполнения бизнес-сценария. Он просто оркестратор.
...
# Делать правильно меня замедлит
Нет. Ради всего святого, это ускорит тебя. 
Ты сможешь запускать тесты без задержек. 
Ты сможешь тестировать бизнес-правила без пользовательского интерфейса. 
Такая гибкость и отсутствие связности всегда тебя ускоряют. 
Если одна вещь, которую мы узнали о связывании за последние пятьдесят лет - 
это то, что связывание лучшее, что может тебя замедлить в разработке.
И связные релизы - это то, что может тебя замедлить.
Знай это.
